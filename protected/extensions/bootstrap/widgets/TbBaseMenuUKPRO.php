...
abstract class TbBaseMenuUKPRO extends CMenu
{
	/**
			...
			foreach ($items as $item)
			{
				...
				/*	if ($count === 1 && $this->firstItemCssClass !== null)
					...
					if (isset($item['items']) && !empty($item['items']))
					{
						...
						{
							$item['linkOptions']['data-target']  = str_replace("#"," ", $item['linkOptions']['data-target']);
							$item['linkOptions']['data-target'] = trim($item['linkOptions']['data-target']);

						}
						else
							$item['linkOptions']['data-target'] = '';


						echo ' <div id="'.$item['linkOptions']['data-target'].'" class="collapse subnav">';


						$this->controller->widget('bootstrap.widgets.TbDropdownUKPRO', array(
	...
	protected function renderMenuItem($item)
	{
		if (isset($item['icon']))
		{
			...
			$item['label'] = '<i class="'.$item['icon'].'"></i> <span>'.$item['label']."</span>";
		...
		if (isset($item['items']) && !empty($item['items']))
		{
			...
			if (isset($item['linkOptions']['class']))
				$item['linkOptions']['class'] .= $item['linkOptions']['class'];
			else
				$item['linkOptions']['class'] = '';

		//	$item['linkOptions']['data-toggle'] = 'dropdown';
			$item['label'] .= ' <b class="fa fa-sort-asc"></b>';
		}
