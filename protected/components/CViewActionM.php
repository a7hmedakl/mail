 ...
 * @since 1.0
 */
class CViewActionM extends CAction
{
	/**
	...
	public function run()
	{
       ...
       $page_url= $this->getRequestedView() ;//.	$controller=$this->getController();
		...
		if(!$event->handled)
		{
			...
			$model= Pages::model()->findByAttributes(array('url'=>$page_url));
			if($model===null)
				throw new CHttpException(404,'The requested page does not exist.');

			$controller->render('staticpage',array('pages'=>$model,'page_url'=>$page_url));
